    ...
    <Accordion.Root type="single" collapsible className="space-y-3">
      {items.map((item, i) => {
        return (
          ...
          >
            <Accordion.Header>
              <Accordion.Trigger className="group flex w-full items-center justify-between gap-4 p-5 text-left text-navy font-semibold hover:bg-navy/5 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-navy focus-visible:ring-inset">
            ...
            <Accordion.Content className="overflow-hidden data-[state=open]:animate-accordion-down data-[state=closed]:animate-accordion-up">
              <div className="px-5 pb-5 text-navy/90">{item.answer}</div>
            </Accordion.Content>
          </Accordion.Item>
        );
      })}
